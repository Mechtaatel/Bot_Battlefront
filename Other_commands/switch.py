    ...
    if self.role == 'Dark':
      ...
      select = Select(options=[
          discord.SelectOption(
              label='104-й Батальон',
              description="Боевое подразделение под командованием Пло Куна.",
              emoji='<:104:1222084458895642717>'),
          discord.SelectOption(label='181-я Танковая дивизия', description=''),
          discord.SelectOption(
              label='212-й Штурмовой батальон',
              description="Штурмовой батальон под командованием Оби-Вана Кеноби"
          ),
          discord.SelectOption(
              label='327-й Звездный корпус',
              description='Боевое подразделение под командованием Аялы Секуры'
          ),
          discord.SelectOption(
              label='41-й Элитный корпус',
              description=
              'Знаменитое подразделение под командованием магистра Йоды'),
          discord.SelectOption(
              label='501-й Легион',
              description=
              'Элитное подразделение под командованием Энакина Скойуокера'),
          discord.SelectOption(label='87-й Корпус часовых', description=''),
          discord.SelectOption(
              label='91-й Разведкорпус',
              description=
              'Мобильный разведкорпус под командование генерала Стасс Алли',
              emoji='<:91:1222093887598891078>'),
          discord.SelectOption(
              label='332-я Рота',
              description=
              'Добровольцы из 501-го неофицально известные как солдаты Асоки',
      ...
      await self.ctx.interaction.response.send_message('Выберите отряд',
                                                       view=view)
      ...
      async def select_callback(interaction):
        ...
        await member.add_roles(
            self.guild.get_role(Roles['C'][select.values[0]]))
