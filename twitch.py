...
with open("default_bot_config.json") as config_file:
...
# Рассчитайте время UNIX на 60 дней в будущем в следующем формате: <t:UNIXTIME:R
def unix_time():
  now = datetime.now()
...
def get_notifications():
  ...
  for user_name in config["twitch"]["watchlist"]:
    ...
    if user_name not in streams:
      ...
      started_at = datetime.strptime(streams[user_name]["started_at"],
                                     "%Y-%m-%dT%H:%M:%SZ")
...
class twitch(commands.Cog):
  ...
  async def check_twitch_online_streamers(self):
    ...
    for notification in notifications:
      ...
      if game == "":
        embed=discord.Embed(title="{}".format(notification["title"]),url="https://twitch.tv/{}".format(notification["user_login"]),description="[Watch](https://twitch.tv/{})".format(notification["user_login"]),
                            color=0x001eff)
        embed.set_author(name=
                "{} Stream ist jetzt Live".format(notification["user_name"]),
        ...
        embed=discord.Embed(title="{}".format(notification["title"]),
                            url="https://twitch.tv/{}".format(notification["user_login"]),
                            description="[Watch](https://twitch.tv/{})".format(notification["user_login"]),
                            color=0x001eff)
        embed.set_author(name=
        "{} Stream ist jetzt Live".format(notification["user_name"]),
                         url="https://twitch.tv/{}".format(notification["user_login"]))
