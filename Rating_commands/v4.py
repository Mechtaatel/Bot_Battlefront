...
class RepeatOrCloseV2(discord.ui.View):
  def __init__(self,ctx,collection,interaction,list_team):
    ...
    self.T21R = collection.find_one({"_id":f"{T21ID}"})["Rating_4v4"]
    self.T22R = collection.find_one({"_id":f"{T22ID}"})["Rating_4v4"]

    self.AuR = collection.find_one({"_id":f"{AuID}"})["Rating_4v4"]
    self.T12R = collection.find_one({"_id":f"{T12ID}"})["Rating_4v4"]
    self.i =0
  ...
  async def repeat(self,button,interaction):
    if interaction.user.id in self.list_team and interaction.user.id not in self.ReaL:
      ...
      if self.i == 4:
        ...
        view2 = Rating_4v4(self.ctx, self.collection,interaction,self.list_team)
...
class button4v4View(View):
  ...
  def __init__(self, ctx, collection,Author, *args, **kwargs):
    ...
    Au = {'id':ctx.author.id,'n':f"{Author['EA_Name']}",'r':Author['Rating_4v4'],'t': 0}
    self.Dictionaries_Team = {'au': Au,'t12':{'id':0,'n': "`___`",'r':0, 't': 0},
                 't13' : {'id':0,'n': "`___`",'r':0, 't': 0},
                 't14' : {'id':0,'n': "`___`",'r':0, 't': 0},
                 't21' : {'id':0,'n': "`___`",'r':0, 't': 1},
                 't22' : {'id':0,'n': "`___`",'r':0, 't': 1},
                 't23' : {'id':0,'n': "`___`",'r':0, 't': 1},
                 't24' : {'id':0,'n': "`___`",'r':0, 't': 1}}
  ...
  async def EloR(self,interaction):
      ...
      if R_a < 100:
        R_a = 100
      R_a = math.ceil(R_a)
      self.collection.update_one({"_id":str(self.value['id'])},{"$set":{"Rating_2v2":R_a}})
      self.interaction = interaction
      if self.j == 8:
        view2 = RepeatOrCloseV2(self.ctx, self.collection,interaction,self.list_team)
  ...
  async def apdate_embed_gameplay(self,interaction):
    ...
    current_embed.set_field_at(1,
       name=f"`{dt['au']['r'] + dt['t12']['r'] + dt['t13']['r'] + dt['t14']['r']}`",
       value=f"""
{dt['au']['p']}`{dt['au']['n']}`
{dt['t12']['p']}`{dt['t12']['n']}`
{dt['t13']['p']}`{dt['t13']['n']}`
{dt['t14']['p']}`{dt['t14']['n']}`""",
    ...
    current_embed.set_field_at(2, 
       name=f"`{dt['t21']['r'] + dt['t22']['r'] + dt['t23']['r'] + dt['t24']['r']}`",
       value=f"""
{dt['t21']['p']}`{dt['t21']['n']}`
{dt['t22']['p']}`{dt['t22']['n']}`
{dt['t23']['p']}`{dt['t23']['n']}`
{dt['t24']['p']}`{dt['t24']['n']}`""",
    ...
    if interaction.user.id in self.list_team:
      if interaction.user.id == self.Dictionaries_Team['au']['id']:
        ...
        if self.Dictionaries_Team['t12']['p'] == '⚫':
          ...
          await interaction.response.apdate_embed_gameplay(totach,ephemeral=True)
  ...
  async def dictonaries_appdate(self,interaction, check_author,ap):
    self.Dictionaries_Team[ap] = {'id':interaction.user.id,
       'n': check_author['EA_Name'],'r':check_author['Rating_4v4']}
    self.list_team.append(interaction.user.id)
    await self.apdate_embed(interaction)
  
  @discord.ui.button(label="join",style=discord.ButtonStyle.grey,custom_id="join1")
  ...
  @discord.ui.button(label="join",style=discord.ButtonStyle.blurple,custom_id="join2")
  ...
  @discord.ui.button(label="exit",style=discord.ButtonStyle.red,custom_id="exit")
